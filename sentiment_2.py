...
# f1 = open("D:/movie_review/new_data_review/The Spy Gone North/baseline_old/ut_0_10_positive_sympathy_3.txt",  "w", encoding='utf-8')
# f2 = open("D:/movie_review/new_data_review/The Spy Gone North/baseline_old/ut_0_10_negative_sympathy_3.txt",  "w", encoding='utf-8')
f3 = open("D:/movie_review/new_data_review/the_outlaws/baseline_old/t_0_10_sympathy_3.txt",  "w", encoding='utf-8')
...
filename1 = "D:/movie_review/new_data_review/the_outlaws/baseline_old/1_10_t.txt"
# 여기에 CSV파일을 불러와서 2번째 리뷰만 포함한 열을 INPUT으로 넣고 돌린다. 또한 코드를 작성할시에 3번째 열에
# 있는 평점을 추가로 작성해주고, 4번째 열의 결과에 따라서 UNL인지 L인지에 따라서 파일을 달리 저장한다. -> sentiment_3.py
...
for line in lines1:
        line2 = line.split(",")[0]
        min3 = line.split(",")[1].replace('\n','')
        print(min3)
        predict_pos_neg(line2.strip())
        min4 = min4 + 1
    # print(line.strip())
